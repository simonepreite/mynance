...
**Verification:** ✅ **CI green on commit `0d564b9`** (run #6) — both jobs pass: ruff `check` + `format --check`, mypy, alembic, pytest (incl. the 2 new problem+json tests), biome, tsc + vite build, and the Playwright theme e2e with the new `--m-*` token assertions. (Run #5 on `38ae982` was red on lint only — ruff ARG001 + biome inner-declarations in the pre-paint script — fixed in `0d564b9`.) The `docker compose up` HMR change (P1) is validated by the build/config but its live runtime check is left for a local `docker compose up` once Docker-users re-login is available, same constraint as the original Docker-gated items.
